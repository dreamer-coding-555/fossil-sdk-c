...
public:
    static void sanitize(std::string &input) {
        fossil_soap_sanitize(&input[0]);
    }

    static bool is_offensive(const std::string &word) {
        return fossil_soap_is_offensive(word.c_str());
    }

    static int32_t count_offensive(const std::string &input) {
        return fossil_soap_count_offensive(input.c_str());
    }
};
