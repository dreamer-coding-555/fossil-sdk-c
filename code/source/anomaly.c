...
double fossil_jellyfish_anomaly_mahalanobis_distance(jellyfish_matrix* vec, jellyfish_matrix* mean, jellyfish_matrix* covariance) {
    jellyfish_matrix* diff = fossil_jellyfish_subtract(vec, mean);
    jellyfish_matrix* trans_diff = fossil_jellyfish_transpose(diff);
    jellyfish_matrix* inv_cov = fossil_jellyfish_inverse(covariance); // Assume fossil_jellyfish_inverse is implemented
    jellyfish_matrix* left_mult = fossil_jellyfish_multiply(trans_diff, inv_cov);
    jellyfish_matrix* result = fossil_jellyfish_multiply(left_mult, diff);
...
void fossil_jellyfish_anomaly_kmeans(jellyfish_matrix* data, int k, int max_iterations, jellyfish_matrix* centroids, int* labels) {
    ...
    for (int iter = 0; iter < max_iterations; iter++) {
        ...
        // Update centroids
        jellyfish_matrix* new_centroids = fossil_jellyfish_create_matrix(k, dims);
...
jellyfish_matrix* fossil_jellyfish_anomaly_identify_outliers(jellyfish_matrix* data, jellyfish_matrix* centroids, double threshold) {
    int n = data->rows;
    jellyfish_matrix* outliers = fossil_jellyfish_create_matrix(n, 1); // 1 column for binary outlier flag
...
void fossil_jellyfish_anomaly_train_autoencoder(jellyfish_matrix* data, jellyfish_layer* encoder, jellyfish_layer* decoder, int epochs, double learning_rate) {
    for (int epoch = 0; epoch < epochs; epoch++) {
        for (int i = 0; i < data->rows; i++) {
            jellyfish_matrix* input = &data[i];
            jellyfish_matrix* encoded = fossil_jellyfish_forward(input, encoder);
            jellyfish_matrix* reconstructed = fossil_jellyfish_forward(encoded, decoder);
            
            // Compute loss and backpropagate
            fossil_jellyfish_mse(input, reconstructed);
            jellyfish_matrix* d_loss = fossil_jellyfish_subtract(reconstructed, input);
...
jellyfish_matrix* fossil_jellyfish_anomaly_detect_with_autoencoder(jellyfish_matrix* data, jellyfish_layer* encoder, jellyfish_layer* decoder, double threshold) {
    jellyfish_matrix* anomalies = fossil_jellyfish_create_matrix(data->rows, 1); // 1 column for binary anomaly flag
