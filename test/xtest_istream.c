...
==============================================================================
*/
#include <fossil/xstdio/istream.h> // library under test

#include <fossil/xtest.h>   // basic test tools
#include <fossil/xassume.h> // extra asserts
#include <fossil/xmock.h>   // mocking tools
...
// Mock input object
static xmock_input_t *mock_input = xnull;
...
XSETUP(input_fixture) {
    ...
    mock_input = xmock_input_create("fossil_console_in_get_input");
    xmock_input_set_inputs(mock_input, 8, "42", "150", "yes", "no", "1 3", "1 4", "exit", "not_exit");
}

XTEARDOWN(input_fixture) {
    xmock_input_erase(mock_input);
}
...
XTEST(reader_let_reader_valid_input_valid) {
    // Test valid input validation
    ASSUME_ITS_TRUE(fossil_console_in_valid_input("Enter a valid number (1-100)", xmock_validate_input));
    ASSUME_ITS_TRUE(xmock_input_verify_call_count(mock_input, 1));
}

XTEST(reader_let_reader_valid_input_invalid) {
    // Test invalid input validation
    ASSUME_ITS_FALSE(fossil_console_in_valid_input("Enter a valid number (1-100)", xmock_validate_input));
    ASSUME_ITS_TRUE(xmock_input_verify_call_count(mock_input, 1));
}

XTEST(reader_let_reader_confirm_yes_no_yes) {
    // Test confirming "yes"
    ASSUME_ITS_TRUE(fossil_console_in_confirm_yes_no("Do you like programming?"));
    ASSUME_ITS_TRUE(xmock_input_verify_call_count(mock_input, 1));
}

XTEST(reader_let_reader_confirm_yes_no_no) {
    // Test confirming "no"
    ASSUME_ITS_FALSE(fossil_console_in_confirm_yes_no("Do you like programming?"));
}

XTEST(reader_let_reader_confirm_multi_menu) {
...
XTEST(reader_let_reader_confirm_exit) {
    // Test confirming exit
    ASSUME_ITS_TRUE(fossil_console_in_confirm_exit());
}

// Additional test cases for reader_valid_input
XTEST(reader_let_reader_valid_input_valid_upper_bound) {
    // Test valid input at upper bound
    ASSUME_ITS_TRUE(fossil_console_in_valid_input("Enter a valid number (1-100)", xmock_validate_input));
}

XTEST(reader_let_reader_valid_input_valid_lower_bound) {
    // Test valid input at lower bound
    ASSUME_ITS_TRUE(fossil_console_in_valid_input("Enter a valid number (1-100)", xmock_validate_input));
}

XTEST(reader_let_reader_valid_input_invalid_upper_bound) {
    // Test invalid input at upper bound
    ASSUME_ITS_FALSE(fossil_console_in_valid_input("Enter a valid number (1-100)", xmock_validate_input));
}

XTEST(reader_let_reader_valid_input_invalid_lower_bound) {
    // Test invalid input at lower bound
    ASSUME_ITS_FALSE(fossil_console_in_valid_input("Enter a valid number (1-100)", xmock_validate_input));
}

// Additional test cases for reader_confirm_yes_no
XTEST(reader_let_reader_confirm_yes_no_invalid_response) {
    // Test invalid response
    ASSUME_ITS_FALSE(fossil_console_in_confirm_yes_no("Do you like programming?"));
}

// Additional test cases for reader_confirm_multi_menu
XTEST(reader_let_reader_confirm_multi_menu_invalid_input) {
...
XTEST(reader_let_reader_confirm_exit_invalid_response) {
